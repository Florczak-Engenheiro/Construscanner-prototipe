...
""", unsafe_allow_html=True)
    ...
    /* Texto padrão - muito escuro para legibilidade */
    body, p, span, div {
    ...
    /* Valor da métrica - preto forte */
    [data-testid="metric-container"] div[style*="color"] {
        color: #000 !important;
    }
...
view_option = st.sidebar.radio(
    "📊 Selecione a visualização:",
    ["Dashboard", "Câmeras", "Alertas", "Relatórios", "Sobre"],
)

# Velocidade de atualização
refresh_speed = st.sidebar.slider(
    "🔄 Velocidade de atualização (segundos):",
    min_value=1,
    max_value=10,
    value=3,
    help="Intervalo para atualizar dados em tempo real"
)

st.sidebar.write("---")
st.sidebar.write("### 📡 Status do Sistema")
col1, col2 = st.sidebar.columns(2)
col1.metric("Câmeras", "6/6", "+6")
col2.metric("Sensores", "12/12", "+12")
...
def gerar_dados_produtividade():
    """Gera dados de produtividade diária"""
    dias = ["Segunda", "Terça", "Quarta", "Quinta", "Sexta"]
    dados = []
    for dia in dias:
        dados.append({
            "Dia": dia,
            "Produtividade": np.random.randint(65, 85),
        })
    return pd.DataFrame(dados)
...
def gerar_alertas():
    ...
    alertas = [
        {
            "tipo": "🔴 Crítico",
            "titulo": "Equipamento Parado por Tempo Excessivo",
            "descricao": "Pá Carregadeira P1 parada há 35 minutos na zona de escavação",
        ...
        },
        {
            ...
            "descricao": "Temperatura ambiente atingiu 28°C - adequar horários",
        ...
        },
        {
            ...
            "descricao": "Produtividade inferior a meta em 8% - revisar cronograma",
        ...
        },
        {
            ...
            "descricao": "Upload de dados completado - 2.3 GB processados",
            "tempo": "1 hora atrás",
            "severidade": "info"
        },
        {
            "tipo": "🔵 Info",
            "titulo": "Novo Trabalhador Identificado",
            "descricao": "Sistema detectou novo trabalhador no canteiro",
            "tempo": "2 horas atrás",
            "severidade": "info"
        },
    ]
...
# ========== HEADER ==========
st.markdown("""
    <div style="text-align: center; padding: 30px 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); margin: -80px -80px 40px -80px; border-radius: 0 0 20px 20px;">
        <h1 style="margin: 0; color: white; font-size: 2.5em;">🏗️ Monitoramento de Obras em Tempo Real</h1>
        <p style="color: rgba(255,255,255,0.9); font-size: 1.1em; margin: 10px 0 0 0;">Sistema inteligente de monitoramento de canteiros com IA</p>
    </div>
""", unsafe_allow_html=True)
...
if view_option == "Dashboard":
    ...
    with col1:
        st.info("""
        **Marcadores no Mapa:**
        - 🟢 **Verde (Ativo)**: Máquina operando normalmente
        - 🟠 **Laranja (Pausado)**: Máquina parada
    ...
    with col1:
        st.bar_chart(df_prod.set_index("Dia"))
    with col2:
        media = df_prod["Produtividade"].mean()
        st.metric("Média Semanal", f"{media:.0f}%", "Produtividade")
...
# ========== CÂMERAS VIEW ==========
elif view_option == "Câmeras":
    st.markdown("### 📷 Monitoramento por Câmeras <span style='background: #1a8c1a; color: white; padding: 5px 10px; border-radius: 10px; font-size: 0.8em; font-weight: bold;'>● AO VIVO</span>", unsafe_allow_html=True)
    
    cameras = [
        {"id": 1, "nome": "Câmera 1", "local": "Entrada Principal", "icon": "📹"},
        {"id": 2, "nome": "Câmera 2", "local": "Zona de Escavação", "icon": "📹"},
        {"id": 3, "nome": "Câmera 3", "local": "Zona de Compactação", "icon": "📹"},
        {"id": 4, "nome": "Câmera 4", "local": "Escritório Temporário", "icon": "📹"},
        {"id": 5, "nome": "Câmera 5", "local": "Acesso Sul", "icon": "📹"},
        {"id": 6, "nome": "Câmera 6", "local": "Vista Aérea (Drone)", "icon": "📹"},
    ]
    ...
    for idx, camera in enumerate(cameras):
        with cols[idx % 3]:
                st.markdown(f"""
                <div style="background: #f0f0f0; border-radius: 8px; padding: 20px; text-align: center; margin: 10px 0;">
                    <div style="font-size: 3em; margin: 20px 0;">{camera['icon']}</div>
                    <h4 style="color: #1a1a1a; font-weight: bold;">{camera['nome']}</h4>
                    <p style="color: #1a1a1a; font-weight: 500;">{camera['local']}</p>
                    <div style="background: #1a8c1a; color: white; padding: 5px 10px; border-radius: 5px; font-size: 0.9em; display: inline-block; font-weight: bold;">
                        ● CONECTADA
                    </div>
                </div>
            """, unsafe_allow_html=True)
...
# ========== ALERTAS VIEW ==========
    ...
    for alerta in alertas:
        ...
        st.markdown(f"""
            <div style="background: {cor_bg}; border-left: 5px solid; padding: 15px; border-radius: 4px; margin: 10px 0; border-left-color: {'#c41c00' if alerta['severidade'] == 'critical' else '#cc7700' if alerta['severidade'] == 'warning' else '#0d47a1'};">
                <h4 style="margin: 0 0 5px 0; color: #1a1a1a; font-weight: bold;">{alerta['tipo']} - {alerta['titulo']}</h4>
                <p style="margin: 5px 0; color: #1a1a1a; font-weight: 500;">{alerta['descricao']}</p>
                <small style="color: #444; font-weight: 500;">{alerta['tempo']}</small>
            </div>
        """, unsafe_allow_html=True)

# ========== RELATÓRIOS VIEW ==========
elif view_option == "Relatórios":
    st.markdown("### 📈 Relatórios e Estatísticas")
    ...
    with col1:
        st.metric("Média de Produtividade", "72%", "+5%")
    with col2:
        st.metric("Máquinas em Operação", "4/5", "-1")
    with col3:
        st.metric("Eficiência do Canteiro", "85%", "+2%")
...
# ========== SOBRE VIEW ==========
elif view_option == "Sobre":
    st.markdown("### ℹ️ Sobre o Sistema")
    
    st.markdown("""
    ## 🏗️ Construscanner
    
    **Sistema Inteligente de Monitoramento de Canteiros de Obras**
    
    ### 🎯 Objetivo
    Monitorar e otimizar operações em canteiros de construção através de Inteligência Artificial, 
    sensores IoT e conectividade avançada.
    ...
    **Pessoal (35% do risco)**
    ...
    ### 🔴 Níveis de Risco
    - 🟢 **SEGURO** (0-40%): Operação normal
    - 🟠 **ALERTA** (40-70%): Risco moderado
    - 🔴 **CRÍTICO** (>70%): Alto risco
    
    ---
    
    **© 2024 Equipe Construscanner | Engenharia & IA**
    """)
